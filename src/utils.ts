    ...
    if (actor.dir === CARDINALS.LEFT || actor.dir === CARDINALS.RIGHT) {
      if (actorBottom > obstacleTop && actorTop < obstacleBottom) {
        if (actor.dir === CARDINALS.LEFT) {
          if (
            actorLeft - actor.speed < obstacleRight &&
            actorRight > obstacleLeft
          ) {
          ...
          if (
            actorRight + actor.speed > obstacleLeft &&
            actorLeft < obstacleRight
          ) {
            if (actor.type === ACTOR_TYPES.PLAYER) {
              actor.spriteRow = 0;
      ...
      if (actorRight > obstacleLeft && actorLeft < obstacleRight) {
        if (actor.dir === CARDINALS.UP) {
          if (
            actorTop - actor.speed < obstacleBottom &&
            actorBottom > obstacleTop
          ) {
            if (actor.type === ACTOR_TYPES.PLAYER) {
              actor.spriteRow = 3;
          ...
          if (
            actorBottom + actor.speed > obstacleTop &&
            actorBottom < obstacleBottom
          ) {
            if (actor.type === ACTOR_TYPES.PLAYER) {
              actor.spriteRow = 2;
...
export const setPosition = ({ actor, game }) => {
  ...
  const actorRight = actor.x + actor.frameWidth;
  const actorTop = actor.y;
  const isBounceLimitMet =
    actor.bounceLimit !== INFINITY && actor.bounceCount >= actor.bounceLimit;
